...
//    ALOGD("Process buffer size: %d", tmpBufSize);
    if (tmpBufSize != 0) {
        static int viperLogCounter = 0;
        if (viperLogCounter % 500 == 0 && tmpBufSize >= 2) {
            ALOGI("ViPER::process[%d] BEFORE effects: size=%d tmpBufSize=%d L0=%.6f R0=%.6f L1=%.6f R1=%.6f",
                  viperLogCounter, size, tmpBufSize, tmpBuf[0], tmpBuf[1], tmpBuf[2], tmpBuf[3]);
        }
        ...
        if (viperLogCounter % 500 == 0 && tmpBufSize >= 2) {
            ALOGI("ViPER::process[%d] AFTER effects, BEFORE limiter: L0=%.6f R0=%.6f L1=%.6f R1=%.6f",
                  viperLogCounter, tmpBuf[0], tmpBuf[1], tmpBuf[2], tmpBuf[3]);
        }
        ...
        if (viperLogCounter % 500 == 0 && tmpBufSize >= 2) {
            ALOGI("ViPER::process[%d] AFTER limiter: L0=%.6f R0=%.6f L1=%.6f R1=%.6f",
                  viperLogCounter, tmpBuf[0], tmpBuf[1], tmpBuf[2], tmpBuf[3]);
            // Also check middle and end of buffer
            uint32_t mid = tmpBufSize;  // middle of the buffer (in samples)
            uint32_t end = (tmpBufSize - 1) * 2;  // last frame
            ALOGI("ViPER::process[%d] AFTER limiter MID[%d]: L=%.6f R=%.6f, END[%d]: L=%.6f R=%.6f",
                  viperLogCounter, mid, tmpBuf[mid], tmpBuf[mid+1], end, tmpBuf[end], tmpBuf[end+1]);
        }
        viperLogCounter++;
